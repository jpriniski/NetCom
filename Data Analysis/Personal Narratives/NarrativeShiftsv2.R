...
# Handle special case for cause_topic
#cause_topic = ifelse(
#  cause_cluster == -1 & grepl("earthquake|earhtquake|quake", cause, ignore.case = TRUE),
#  "Earthquake",
#  cause_topic
#    )#)

# Map topic labels and apply post-processing
claims_full <- claims_full %>%
  mutate(
    # Map cluster IDs to topics using topic_labels
    cause_topic = topic_labels[as.character(cause_cluster)],
    ...
    # Handle special case for effect_topic
    #effect_topic = ifelse(
    #  effect_cluster == -1 & grepl("earthquake|earhtquake|quake", effect, ignore.case = TRUE),
    #  "Earthquake",
    #   effect_topic
    #)
  ) %>%
  ...
  mutate(
    cause_topic = case_when(
      ...
      #cause_topic %in% c("Energy Shortage", "Change (Loss)") ~ "Energy Shortage" , #HJ current coding, comment out to make setsuden own category
    ...
    effect_topic = case_when(
      ...
      #effect_topic %in% c("Energy Shortage", "Change (Loss)") ~ "Energy Shortage" , #HJ current coding, comment out to make setsuden own category
      effect_topic %in% c("Health Issues", "Cancer") ~ "Health Issues" ,
      TRUE ~ effect_topic
    )
  )

#original analysis computes using cause_cluster, effect_cluster; updated (remapped) does on _topic
...
claim_counts2 %>%
  ...
  summarise(
    mean_row_count = mean(row_count),
    median_row_count = median(row_count),
    mode_row_count = row_count[which.max(tabulate(match(row_count, unique(row_count))))],
    sd_row_count = sd(row_count)
  )


#882 rows , is 21*21 (topic pairs) times 2, one pre one post
...
#pvals = c(0.01, 0.04, 0.03, 0.20, 0.001, 0.05)
#p.adjust(pvals, method="BH", n = 6)
...
write_csv(shift_t_tests, 'shift_t_tests.csv')
...
#Make a shift plot for each of the causal relations, include in the Sup materials

shift_df3 <- shift_df2%>%
  mutate('Shift' = unique_positive_count - unique_negative_count,
         cause_cluster = recode(as.character(cause_cluster), !!!topic_labels),
         effect_cluster = recode(as.character(effect_cluster), !!!topic_labels))


homo_hash_shift <- shift_df3%>%
  filter(Structure == 'Homogeneous')%>%
  filter(Content == 'Hashtag')

homo_face_shift <- shift_df3%>%
  filter(Structure == 'Homogeneous')%>%
  filter(Content == 'Face')

spat_hash_shift <- shift_df3%>%
  filter(Structure == 'Spatial')%>%
  filter(Content == 'Hashtag')

spat_face_shift <- shift_df3%>%
  filter(Structure == 'Spatial')%>%
  filter(Content == 'Face')

#write_csv(homo_hash_shift, "homo_hash_shift.csv")
#write_csv(spat_hash_shift, "spat_hash_shift.csv")
